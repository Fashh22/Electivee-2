    ...
    if (mb_strlen($phone) > 40) {
        ...
        if ($newPassword !== '' || $confirmPassword !== '') {
            if (strlen($newPassword) < 8) {
                ...
                $pwStmt = $pdo->prepare("SELECT password_hash FROM users WHERE id = ? AND role = 'student'");
                $pwStmt->execute([$studentId]);
                $pwRow = $pwStmt->fetch();
...
<div class="teacher-layout">
    <aside class="teacher-sidebar">
        ...
        <a href="student_dashboard.php#section-overview" class="teacher-nav-item" data-target="section-overview">Overview</a>
        ...
        <a href="student_profile.php" class="teacher-nav-item teacher-nav-profile active">Profile</a>
    </aside>
    ...
    <main class="teacher-main">
        ...
        <div class="container wide teacher-wrap">
            ...
            <div class="card teacher-profile-page">
                <h3 class="teacher-card-title">Student Profile</h3>
                <form method="post" class="teacher-form" id="studentProfileForm">
                    <input type="hidden" name="action" value="update_profile">
                    <label>Full Name</label>
                    <input type="text" name="name" value="<?php echo e($student['name'] ?? ''); ?>" required autocomplete="name">
                    <label>Email</label>
                    <input type="email" name="email" value="<?php echo e($student['email'] ?? ''); ?>" required autocomplete="email">
                    <label>Phone</label>
                    <input type="text" name="phone" value="<?php echo e($student['phone'] ?? ''); ?>" maxlength="40" autocomplete="tel" placeholder="Optional">
                    <label>Address</label>
                    <textarea name="address" rows="2" maxlength="255" placeholder="Optional"><?php echo e($student['address'] ?? ''); ?></textarea>
                    <p class="request-sub teacher-profile-meta">Member since: <?php echo e($student['created_at'] ?? '-'); ?></p>
                ...
                <div class="teacher-profile-actions">
                    <button class="button success" type="submit" form="studentProfileForm">Save Profile</button>
